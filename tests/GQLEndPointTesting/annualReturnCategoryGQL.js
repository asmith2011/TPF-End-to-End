...
export default function () {
  ...
  group('Annual Return Datapoints Validation', function () {
    for (var i = 0; i < input.length; i++) {
      const obj = JSON.parse(JSON.stringify(input[i]));
      var identifier = obj.Name;
      var isAnnualReturnCategoryEnabled = obj.isAnnualReturnCategoryEnabled;
      var startYear = obj.startYear;
      var endYear = obj.endYear;

      if (isAnnualReturnCategoryEnabled === 'true') {
        const query = `query {
      annualReturnCategory( startYear:"${startYear}", endYear:"${endYear}", identifier: "${identifier}") {
      reportDate
      totalAnnualReturn
      } }`;
        ...
        for (let i = 0; i < body.data.annualReturnCategory.length; i++) {
          let reportDate = body.data.annualReturnCategory[i].reportDate;
          let totalAnnualReturn = body.data.annualReturnCategory[i].totalAnnualReturn;
...
export function handleSummary(data) {
  return {
    "GQLReports/annualReturnCategoryGQL Test.html": htmlReport(data),
